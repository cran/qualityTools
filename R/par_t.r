paretoPlot = function(fdo, threeWay = FALSE, abs = TRUE, decreasing = TRUE, na.last = NA, 
    alpha = 0.05, xlim, ylim, xlab, ylab, main, ...) {
    DB = FALSE
    ylimMissing = FALSE
    if (missing(ylim)) 
        ylimMissing = TRUE
    ...
    for (j in 1:ncol(response(fdo))) {
        ...
        if (!any(is.na(response(fdo)[, j]))) {
            ...
            if (df.resid == 0) {
                ...
                effect = ((summary(lm.1)$coefficients[-pmatch("(Intercept)", names(coef(lm.1))), 
                  1])/(summary(lm.1)$coefficients[-pmatch("(Intercept)", names(coef(lm.1))), 2]))
                if (all(is.na(effect))) 
