#calculates alpha for orthogonality (solely)
#k - number of factors
#p - number of factors attended by confounding
#cc - number of centerCube Points (per block!)
#cs - number of centerStar Points
...
#calculates alpha for rotatability (solely)
#k - number of factors
#p - number of factors attended by confounding
...
#helper function - creates a centerpoints (0) frame with nrow rows and k columns
#nrow - number of rows
#k - number of factors
...
#helper function - creates a star data.frame for a given number of factors k and the value of alpha
#k - number of factors
...
#blocks - number of blocks for ccd Design
#alpha - "rotatable", "orthogonal" or "both". If "both" values for cc and cs will be discarded
#cc - number of centerCube Points (per block!)
#cs - number of centerStar Points
rsmDesign = function(k = 3, p = 0, alpha = "rotatable", blocks = 1, cc = 1, cs = 1, faceCentered = FALSE)
{
  ...
  #add center points in cube portion
  if(cc > 0)
  {
  ...
  #add center points in star portion
  if(cs > 0)
  {
...
#rsmDesign(k = 3, blocks = 1, alpha = "both")



#######Useful Central Composite Designs with orthogonal Blocking and near rotatability
###Base Desings taken from rsm 2009 Montgomery p.329, table 7.5
...
rsmChoose = function()
{
  ...
  numRows = 6 #i.e. 1,2,3,5,9 or 17 blocks
  numCol = 9 #i.e. 2, 3, 4, ,5 , 5-1,  6, 6-1, 7, 7-1
  blockVals = c(1,2,3,5,9,17)  #nur diese werden berücksichtigt
  ...
  #necessary to run through R CMD BUILD (cancels locator process)
  if(length(x) < 1)
