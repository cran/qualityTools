.m.interaction.plot = function(x.factor, trace.factor, response, fun = mean, type = c("l", "p", "b"), 
    legend = TRUE, trace.label = deparse(substitute(trace.factor)), fixed = FALSE, xlab = deparse(substitute(x.factor)), 
...
setGeneric("effectPlot", def = function(object, factors, fun = mean, response = NULL, single = FALSE, 
    points = FALSE, classic = FALSE, axes = TRUE, lty, xlab, ylab, main, ylim, ...) standardGeneric("effectPlot"))
setMethod(effectPlot, signature(object = "facDesign"), function(object, factors, fun = mean, response = NULL, 
...
setMethod(effectPlot, signature(object = "taguchiDesign"), function(object, factors, fun = mean, response = NULL, 
    single = FALSE, points = FALSE, classic = FALSE, axes = TRUE, lty, xlab, ylab, main, ylim, ...) {
    oldMar = par("mar")
    oldOma = par("oma")
    oldMfrow = par("mfrow")
...
snPlot = function(object, type = "nominal", factors, fun = mean, response = NULL, single = FALSE, points = FALSE, 
    classic = FALSE, axes = TRUE, lty, xlab, ylab, main, ylim, ...) {
    Debugging = TRUE
    ...
    y = numeric()
    if (missing(main)) {
        for (k in 1:resLength) m[k] = paste("Effect Plot for S/N ratios of", names(response(object))[k])
        main = m
    }
    if (missing(ylab)) {
        for (k in 1:resLength) y[k] = paste("means of S/N ratios for ", names(response(object))[k])
        ylab = y
    }
    ...
    for (k in 1:resLength) {
        ...
        if (k > 1) 
            dev.new()
        effectPlot(object = tdo, factors = factors, fun = mean, response = response, single = single, 
            points = points, classic = classic, axes = axes, lty = lty, xlab = xlab, ylab = ylab[k], 
            main = main[k], ylim = ylim, ...)
    }
