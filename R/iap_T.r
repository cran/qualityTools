...
interactionPlot = function(fdo = NULL, y = NULL, response = NULL, 
    fun = mean, main = paste("Interaction plot for ", deparse(substitute(fdo))), 
    col = 1:2, ...) {
    DB = FALSE
    parList = list(...)
    ...
    if (class(fdo) != "facDesign") {
        ...
        for (i in 1:ncol(combMat)) {
            ...
            temp = with(cbind(x, y), .testFun(eval(parse(text = facName2)), 
                eval(parse(text = facName1)), response = y, trace.label = facName1, 
                ylim = range(y), axes = F, fun = fun, title = facName1, 
                col = col, ...))
            if (colNum == numFac) {
                tempList = parList
                tempList$col = 1
