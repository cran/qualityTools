cg = function(x, target, tolerance, ref.interval, facCg, facCgk, n = 0.2, type, col, pch, xlim, 
    ylim, conf.level = 0.95, cex.val = 1.5) {
    if (missing(x)) 
    ...
    if (missing(tolerance)) 
        warning("Missing T! The specification limits are choosen to get Cg = 1")
    if (missing(tolerance)) {
        width = ref.ar/facCg
        tolerance = numeric(2)
    ...
    plot(x, type = plot.type, col = col, main = "RunChart", pch = pch, xlim = c(xlim[1] - 0.05 * xlim[2], xlim[2]), 
        ylim = ylim)
    abline(h = target)
    ...
    temp = axis(4, at = c(target + n/2 * (abs(diff(tolerance))), target - n/2 * (abs(diff(tolerance))), mean(x)), 
        labels = FALSE)
    text(max(xlim) + 0.075 * max(xlim), temp[3], srt = 0, adj = 0, substitute(x[tar] + a %.% b, list(a = round(n/2, 
        4), b = "T")), xpd = TRUE, cex = cex.val)
    text(max(xlim) + 0.075 * max(xlim), temp[2], srt = 0, adj = 0, expression(bar(x)), xpd = TRUE, cex = cex.val, 
        col = "seagreen")
    ...
    text(max(xlim) + 0.075 * max(xlim), quant1, srt = 0, adj = 0, substitute(x[a * b], list(a = lt1, b = "%")), 
        xpd = TRUE, cex = cex.val, col = "seagreen")
    text(max(xlim) + 0.075 * max(xlim), quant2, srt = 0, adj = 0, substitute(x[a * b], list(a = lt2, b = "%")), 
        xpd = TRUE, cex = cex.val, col = "seagreen")
    par(mar = par.old)
    ...
    plot(x, type = plot.type, col = col, main = "Tolerance View", pch = 20, xlim = xlim, ylim = c(min(x, tolerance[2], 
        tolerance[1], target + n/2 * (tolerance[2] - tolerance[1]), target - n/2 * (tolerance[2] - tolerance[1])), 
        max(x, tolerance[2], tolerance[1], target + n/2 * (tolerance[2] - tolerance[1]), target - n/2 * (tolerance[2] - 
            tolerance[1]))))
