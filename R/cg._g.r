...
cgRunChart=function(x, target, tolerance, ref.interval, facCg, facCgk, n = 0.2, type, col, pch, xlim, ylim,main, conf.level = 0.95, cex.val = 1)
{
    if (missing(x)) 
        stop("x must be given as a vector")
    if (missing(target)) {
...
cgHist=function(x, target, tolerance, ref.interval, facCg, facCgk, n = 0.2, col, xlim, ylim, main, conf.level = 0.95, cex.val = 1)
{
    if (missing(x)) 
        stop("x must be given as a vector")
    if (missing(target)) {
...
cgToleranceView=function(x, target, tolerance, ref.interval, facCg, facCgk, n = 0.2, type, col, pch, xlim, ylim, main, conf.level = 0.95, cex.val = 1)
{
    if (missing(x)) 
        stop("x must be given as a vector")
    if (missing(target)) {
...
cg = function(x, target, tolerance, ref.interval, facCg, facCgk, n = 0.2, type, col, pch, xlim, ylim, conf.level = 0.95, cex.val = 1.5) {
    ...
    cgHist(x=x, target=target, tolerance=tolerance, ref.interval=ref.interval, 
           ...
           conf.level = conf.level, cex.val = cex.val)
    par(mar = par.old)
    par(mar = c(3.1, 4.1, 1.55, 2.1))
    cgToleranceView(x=x, target=target, tolerance=tolerance, ref.interval=ref.interval, 
                    facCg=facCg, facCgk=facCgk, n = n, type=type, col=col, pch=pch, 
                    xlim=xlim, ylim=ylim, main="Tolerance View", conf.level = conf.level, cex.val = cex.val)
    par(mar = par.old)
 invisible(list(Cg, Cgk))
} 
