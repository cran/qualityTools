paretoChart = function(x, weight, showTable = TRUE, las = 0, main, col, border, xlab, 
    ...
    if (length(xtable) > 1) {
        ...
        if (showTable) {
            ...
            mymai[3] = 0
            par(mai = mymai)
            plot(xValue, rep(1, length(xValue)), xlim = c(0.2, 1.2 * length(xtable)), ylim = c(0, 
                5), axes = FALSE, ylab = "", type = "n")
            axis(2, pos = 0.2, at = 1:4, labels = rev(c(ylab, paste("Cum.", ylab), "Percentage", 
                "Cum. Percentage")), tick = FALSE, las = 1)
