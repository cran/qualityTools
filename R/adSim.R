...
# Fehlerprüfungen durchführen

# 1.20 Überprüfung, dass x ein numerischer Vektor ist
		if(mode(x) != "numeric")
			stop(paste("\n","          adSim() requires numeric x data"))	
			
# 1.25 Überprüfung von x auf fehlende Werte
		if(any(is.na(x))) 
			stop(paste("\n","          x data has missing values (NA)"))
	
# 1.30 Überprüfung, ob auf die angegebene Verteilung getestet werden kann
		
			# Vektor "distr" enthält die testbaren Verteilungsmodelle
...
# 1.35 & 1.40 Überprüfung, ob die gewählte Verteilung für Werte von x<=0 definiert ist bzw. der MLE berechnet werden kann, wenn x entsprechende Werte hat
...
# 1.45 & 1.50: bei log-Normalverteilung wird die Stichprobe logarithmiert und anschließend auf Normalverteilung getestet
...
# 1.54 & 1.56 & 1.58: bei Weibull-Verteilung wird die Stichprobe tranformiert x=-log(x) und anschließend auf Gumbel-Verteilung getestet
# obwohl die Weibull-Verteilung für x>=0 definiert ist, muss aufgrund der Logarithmierung x>0 gelten (die Prüfung erfolgt bereits mit 1.35 & 1.40)
...
# 1.80 Prüfung auf eine Mindeststichprobengröße
...
###################################################################
# 2.00 Parameterschätzung
###################################################################


# 2.10 & 2.15 Zum Schätzen der Parameter standardmäßig den MLE benutzen (außer für die Normal-, Cauchy- und Gumbel-Verteilung)
# Parameter der Verteilung aus Stichprobe schätzen und als Liste "parafit" speichern
...
# 2.30 & 2.35 Schätzen der Parameter der Normalverteilung 
# der MLE ist für die Standardabweichung der Normalverteilung nicht erwartungstreu
# Parameter der Normalverteilung werden mit mean() und sd()  geschätzt und als Vektor "parafit" gespeichert	
...
# 2.50 Schätzen der Cauchy-Parameter
# Für die Cauchy-Verteilung sind zwei unterschiedliche Schätzverfahren implementiert

	if(testDistr == "cauchy"){

		# 2.52 Prüfung, ob simuliert wird oder kritische Werte ausgelesen werden sollen	
		
		if(is.na(b) == FALSE){		# die AD-Verteilung soll später simuliert werden
			
			# 2.55 Schätzung mittels fitdistr()	
				
				library(MASS)
				parafit <- fitdistr(x,testDistr)
				
		}else{						# 2.52 die AD-Verteilung soll später nicht simuliert werden			
			
		# 2.60 Parameterschätzung, basierend auf den Summen der gewichteten Ordnungsstatistik
			...
			#  für ungerade n können nicht alle Gewichte berechnet werden, deshalb wird das fehlende Gewicht geschätzt
			if(ifelse(n %% 2, TRUE, FALSE)){ 	# ifelse-Bedingung gibt TRUE, wenn n ungerade ist
				if(length(na.omit(uWeight)) + 1 == length(uWeight)){  # Bedingung prüft, ob nur ein Wert in uWeight nicht berechnet werden konnte
				
				 uWeight[which(is.na(uWeight))] = (n+1)/n - sum(na.omit(uWeight))   # Ersetzen von NaN durch geeigneten Schätzer 
				}}				
...
# 2.70 Schätzen der Gumbel-Parameter (fitdistr() kann nicht die Gumbel-Verteilung schätzen)

	if(testDistr == "gumbel"){
		
				### 2.72 p-Gumbel-Funktion definieren (aus Library VGAM übernommen)
				...
				### 2.75 r-Gumbel-Funktion definieren (aus Library VGAM übernommen)
				...
				### 2.80 MLE-Parameterschätzung für die Gumbel-Verteilung (iterative Lösung)
			
				f <- function(p) 1/n*sum(x) - (sum(x*exp(-x/p)))/(sum(exp(-x/p)))-p
					
					### iterative Lösung notwendig, um beta zu schätzen (D'Agostino/Stephens 1986, S. 146)
...
###################################################################
	...
	# 3.10 Transformation des Wahrscheinlichkeitsintegrals (PIT) berechnen
 	...
 	# 3.50 Anwendung der AD-Formel
			...
			# 3.60 für jedes i den Summenwert berechnen und in Matrix h schreiben
			h = matrix(ncol=n)

			h[1,] = (2*col(h)-1)*log(pit) + (2*n + 1 - 2*col(h))* log(1 - pit) 
			
			# 3.70 AD-Formel vollständig berechnen
			AD = -n-(1/n) * rowSums(h)
		

			# 3.80 Fehlerprüfung, ob AD-Wert vorhanden ist
			if(is.na(AD) == TRUE || AD<0)
				stop(paste("\n","        The calculation of the Anderson Darling statistic fails."))
	
			# Ende der AD-Wert-Berechnung für die beobachteten Daten
...
#####################################################################################
	...
	# 4.00 Fehlerprüfung, ob die Simulationshäufigkeit zulässig gewählt ist
	...
	# 4.05 Resampling und Parameterschätzung
	# Matrix Y mit b sortierten Zufallsstichproben mit Umfang n erstellen
	# Verteilungsparameter zu jeder Bootstrap-Stichprobe schätzen	
...
# PIT für jeden Wert der Matrix Y berechnen
# (abhängig von den geschätzten Verteilungsparameter jeder Bootstrap-Stichprobe)
...
# 4.75 Berechnung der Summenglieder der AD-(Lewis)-Formel zu jeder Bootstrap-Stichprobe (Matrix Y überschreiben)
...
# 4.80 Fehlerprüfung, ob alle AD-Werte bestimmt werden konnten
if(any(is.na(simAD))){
...
# Ende der simulationsbasierten Bestimmung von kritischen Werten bzw. des p-Werts
		...
		# 5.02 simAD wird später in der Ausgabe abgerufen, weshalb die Variable definiert sein muss
		simAD = NA
		...
		# 5.08 Test auf Normal-, Exponential-, Gumbel- oder logistische Verteilung
			...
			# 5.10 Test auf Normalverteilung
				if(testDistr == "normal"){
					...
					###################################################################################################################
					### kritische Werte für die Normalverteilung nach D'Agostino und Stephens 1986 - S. 123   Tab. 4.7 
					################################################################################################################### 					
					...
					# 5.14 Anpassen der Wertetabelle bezüglich des Stichprobenumfangs
					normalMtx[2,1:ncol(normalMtx)] = normalMtx[2,1:ncol(normalMtx)]/(1+0.75/n+2.25/n^2)
					refMtx = normalMtx
					
					########################################################################################################
					### p-Wert Berechnung für die Normalverteilung nach D'Agostino und Stephens 1986 - S. 127    
					######################################################################################################## 
				...
				if(testDistr == "exponential"){
					...
					#############################################################################################################
					### Kritische Werte für die Exponentialverteilung nach D'Agostino und Stephens 1986 - S. 135   Tab. 4.11   
					############################################################################################################# 					
					...
					# 5.24 Anpassen der Wertetabelle bezüglich des Stichprobenumfangs
						expMtx[2,1:ncol(expMtx)] = expMtx[2,1:ncol(expMtx)]/(1+0.6/n)
	
						refMtx = expMtx
				
					############################################################################################################
					### p-Wert-Berechnung für die Exponentialverteilung nach D'Agostino und Stephens 1986 - S. 136
			...
			if(testDistr == "gumbel" || testDistr == "logistic"){			
				...
				if(testDistr == "gumbel"){
					...
					##############################################################################################################
					### Kritische Werte für die Extremwert-Verteilung nach D'Agostino und Stephens 1986 - S. 146   Tab. 4.17   
					############################################################################################################## 					
					...
					# 5.34 Anpassen der Wertetabelle bezüglich des Stichprobenumfangs
				...
				if(testDistr == "logistic"){
					...
					##############################################################################################################
					### kritische Werte für die logistische Verteilung nach D'Agostino und Stephens 1986 - S. 157   Tab 4.22 
					############################################################################################################## 
					...
					# 5.39 Anpassen der Wertetabelle bezüglich des Stichprobenumfangs
				...
				##############################################################################################################
				### Bestimmung des p-Werts für die Gumbel- oder logistische Verteilung
				############################################################################################################## 
			
					critCheck <- refMtx[2,1:ncol(refMtx)] > AD   ### gibt TRUE aus für alle Einträge in der Zeile, die größer als AD sind
					
						# 5.40 Existiert ein kritischer Wert, der größer als der AD-Wert ist?   	
						if(any(critCheck)){
							...
							# 5.44 letzte Spalte als Position wählen
								firPos <- ncol(refMtx)}
				...
				##############################################################################################################
				### Auslesen der kritischen Werte für Normal-, Exponential-, Gumbel oder logistische Verteilung				
				############################################################################################################## 
												
					for(i in 1:ncol(critValues)){
						
						# 5.50 Ist der kritische Werte für das zu spezifizierende Quantil tabelliert?
						if(any(refMtx[1,1:ncol(refMtx)] == critValues[1,i])){                          
							...
							# 5.54 Auslesen des kritischen Werts bezüglich der Position	
								critValues[2,i] <- refMtx[2,position]          ### liest aus der Matrix den kritschen Wert abhängig vom gewählten Quantil
						}else{
	...
	if(testDistr == "gamma"){
					...
					###################################################################################################################################
					### Bestimmung des kritischen Werts und des p-Werts für die Gammaverteilung nach D'Agostino und Stephens 1986 - S. 155 - Tab. 4.21
					...
					###################################################################################################################################
					######## p-Wert-Bestimmung für die Gamma-Verteilung
					###################################################################################################################################
					
						# critCheck gibt TRUE aus für alle Einträge in der entsprechenden m-Zeile, die größer als der AD-Wert sind
						# zu beachten ist, dass die Betrachtung erst ab der 2ten Spalte der Tabelle erfolgt, so dass die Indizierung von critCheck
						# gegenüber der Indizierung der Tabelle  um -1 verschoben ist 
						
							critCheck <- gammaDF[min(which(gammaDF$m >= parafit$estimate["shape"])),2:ncol(gammaDF)] > AD   
								 
						# 5.65 Existiert ein kritischer Wert, der größer als der AD-Wert ist (in der entsprechenden m-Zeile)?  
							if(any(critCheck)){	
								
								# 5.66 firPos gibt die Spalten-Position der Tabelle in der entsprechenden m-Zeile von dem krit. Wert an, 
								# der noch kleiner als der AD-Wert ist. Für firPos = 1 existiert kein kleiner kritischer Wert
								
									firPos <- min(which(critCheck))     
							}else{
								
							# 5.67 letzte Spalte als Position wählen
									firPos <- ncol(gammaDF) }
					...
					###################################################################################################################################
					######## kritischen Wert für die Gamma-Verteilung auslesen
					###################################################################################################################################
						  
							
							for(i in 1:ncol(critValues)){
								
								# 5.70 Ist der kritische Wert für das zu spezifizierende Quantil tabelliert?
	...
	if(testDistr == "cauchy"){
					...
					####################################################################################################################
					### Bestimmung des kritischen Werts für die Cauchy-Verteilung nach D'Agostino und Stephens 1986 - S. 163   Tab 4.26
					...
					##############################################################################################################
					### Bestimmung des p-Werts für die Cauchy-Verteilung 
					############################################################################################################## 
				
					# 5.84 Existieren kritische Werte für den zu untersuchenden Stichprobenumfang n?
					if(any(cauchyDF[1:13,1] == n)){						
					
						# critCheck gibt TRUE aus für alle Einträge in der entsprechenden n-Zeile, die größer als der AD-Wert sind
						# zu beachten ist, dass die Betrachtung erst ab der 2ten Spalte der Tabelle erfolgt, so dass die Indizierung von critCheck
						# gegenüber der Indizierung der Tabelle um -1 verschoben ist				
								critCheck <- cauchyDF[which(cauchyDF[1:13,1] == n),2:ncol(cauchyDF)] > AD
						
						# 5.85 Existiert ein kritischer Wert, der größer als der AD-Wert ist (in der entsprechenden n-Zeile)? 
							if(any(critCheck)){	
								...
								# 5.87  letzte Spalte als Position wählen 	
					...
					##############################################################################################################
					### Kritische Werte für die Cauchy-Verteilung auslesen 
					############################################################################################################## 
					
											
							for(i in 1:ncol(critValues)){
								
								# 5.90 Ist der kritische Wert für das zu spezifizierende Quantil tabelliert?
						...
						# 5.96 p-Wert kann nicht ermittelt werden, da für "n" keine tabellierten Werte existieren
							pValue <- NA
						
						# 5.98 kritische Werte können nicht ermittlet werden, da für "n" keine tabellierten Werte existieren
...
###########################################################################
####	6.00 Parameterkorrektur für die Weibull-Verteilung
###########################################################################
